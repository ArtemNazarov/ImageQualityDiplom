...
function decorateFireplace() {
    ...
    FIRE_RECT_SIDE = Math.round(bounds.getHeight() / 20);
    SMOKE_RECT_SIDE = Math.round(0.8 * FIRE_RECT_SIDE);
    var centerX = bounds.getLeft() + bounds.getWidth() / 2;
    var bottom = bounds.getTop() + bounds.getHeight();
    var top = bounds.getTop() + Math.round(bounds.getHeight() / 1.7);
    stones.clear();
...
function getFireStartPoint() {
    ...
    return acgraph.math.coordinate(left, top);
}
